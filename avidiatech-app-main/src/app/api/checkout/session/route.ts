  ...
  try {
    ...
    if (!priceId || typeof priceId !== "string" || !priceId.startsWith("price_")) {
      ...
      console.error("checkout: missing or invalid Stripe price id", {
        ...
        envMap,
        hint: "Ensure STRIPE_PRICE_ID_<PLAN>_MONTHLY and STRIPE_PRICE_ID_<PLAN>_YEARLY are set in the environment.",
      });
    ...
    if (email) {
      try {
        ...
        if (customers.data[0]) {
          ...
          // Ensure clerkUserId is on the customer metadata for webhook lookups
          if (!customer.metadata?.clerkUserId) {
            customer = await stripe.customers.update(customer.id, {
              metadata: { clerkUserId: userId, tenant_id: userId },
            });
          }
        }
      } catch (err) {
    ...
    // Derive plan name for metadata (used by webhook to set quotas)
    const planName = (body?.plan || "starter").toString().toLowerCase();
    ...
    try {
      const sessionParams: any = {
        ...
        subscription_data: {
          ...
          metadata: { clerkUserId: userId, billing, plan: planName },
        },
        metadata: { clerkUserId: userId, billing, plan: planName },
