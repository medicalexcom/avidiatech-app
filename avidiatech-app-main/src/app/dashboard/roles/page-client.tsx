              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800/60">
                {members.map((m) => (
                  <div key={m.id} className="flex items-center gap-3 px-4 py-3">
                    {/* Avatar */}
                    <div className={`h-8 w-8 shrink-0 rounded-full ${ROLE_AVATAR_COLORS[m.role]} flex items-center justify-center text-[12px] font-bold text-white`}>
                      {m.avatar}
                    </div>
                    ...
                    {/* Role badge / selector */}
                    {m.role === "owner" ? (
                      <span className={`rounded-full border px-2 py-0.5 text-[12px] font-semibold uppercase ${ROLE_COLORS[m.role]}`}>
                        Owner
                      </span>
                    ) : (
                      <select
                        ...
                        className={`rounded-full border px-2 py-0.5 text-[12px] font-semibold uppercase cursor-pointer ${ROLE_COLORS[m.role]} bg-transparent disabled:opacity-60`}
                      >
                        <option value="admin">Admin</option>
                        <option value="member">Member</option>
                        <option value="viewer">Viewer</option>
                    ...
                    {/* Last active */}
                    <span className="text-[12px] text-slate-400 dark:text-slate-600 whitespace-nowrap hidden sm:block">
              ...
              {(["owner", "admin", "member", "viewer"] as Role[]).map((role) => {
                ...
                return (
                  <div key={role} className={`rounded-xl border px-3 py-2.5 ${ROLE_COLORS[role]}`}>
                    ...
                    <p className="mt-0.5 text-[12px] opacity-80">{desc[role]}</p>
                  </div>
                );
              })}
            </div>
