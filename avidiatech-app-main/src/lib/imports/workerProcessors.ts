...
async function insertRowsBatch(rows: any[]) {
  ...
  try {
    await getServerSupabase().from("import_rows").insert(rows);
  } catch (err) {
...
async function logModule(pipelineRunId: string | null, moduleIndex: number, level: string, message: string, meta?: any) {
  ...
  try {
    await getServerSupabase().from("pipeline_module_logs").insert({
...
export async function connectorSyncProcessor(data: { integrationId: string; jobId: string }) {
  ...
  await getServerSupabase().from("import_jobs").update({ status: "processing", updated_at: new Date().toISOString() }).eq("id", jobId);
  await logModule(pipelineRunId, moduleIndex, "info", "connectorSync started", { integrationId });

  const { data: integration } = await getServerSupabase().from("integrations").select("*").eq("id", integrationId).single();
  if (!integration) {
    await logModule(pipelineRunId, moduleIndex, "error", "integration not found");
    await getServerSupabase().from("import_jobs").update({ status: "failed", updated_at: new Date().toISOString(), last_error: "integration not found" }).eq("id", jobId);
    return;
  }
  ...
  try {
    for await (const product of adapter.paginateProducts()) {
      ...
      if (rowsBuffer.length >= BATCH_SIZE) {
        ...
        inserted += BATCH_SIZE;
        await getServerSupabase().from("import_jobs").update({ meta: { processed: inserted } }).eq("id", jobId);
    ...
    await getServerSupabase().from("import_jobs").update({ status: "completed", updated_at: new Date().toISOString() }).eq("id", jobId);
    ...
    await getServerSupabase().from("import_jobs").update({ status: "failed", updated_at: new Date().toISOString(), last_error: String(err?.message ?? err) }).eq("id", jobId);
  }
}
...
export async function importProcessProcessor(data: { jobId: string }) {
  ...
  const { data: jobRow } = await getServerSupabase().from("import_jobs").select("*").eq("id", jobId).single();
  if (!jobRow) return;
  const filePath = jobRow.file_path;
  if (!filePath) {
    await logModule(pipelineRunId, moduleIndex, "error", "missing file_path in import_job");
    await getServerSupabase().from("import_jobs").update({ status: "failed", last_error: "no file_path" }).eq("id", jobId);
    return;
  }
  ...
  try {
    const storage = getServerSupabase().storage.from(jobRow.meta?.bucket ?? "imports");
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      parse(stream as any, {
        ...
        error: async (err) => {
          ...
          await getServerSupabase().from("import_jobs").update({ status: "failed", last_error: String(err?.message ?? err) }).eq("id", jobId);
          reject(err);
        },
      });
    });
