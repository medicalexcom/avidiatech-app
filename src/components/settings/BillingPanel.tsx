...
export default function BillingPanel() {
  ...
  async function openPortal() {
    ...
    try {
      ...
      if (r.ok && json?.url) {
        // If reason is no_stripe_customer the API redirects to pricing page
  ...
  const ing     = summary.usage?.ingests      ?? { used: 0, limit: null };
  const tr      = summary.usage?.translations ?? { used: 0, limit: null };

  /** Display "X / Unlimited" when limit is null (owner or Pro plan) */
  function fmtUsage(used: number, limit: number | null) {
              ...
              <div>
                <div className="flex items-center justify-between text-[12px]">
                  ...
                  <span className="tabular-nums text-slate-500 dark:text-slate-400">
                    {fmtUsage(ing.used, ing.limit)}
                  </span>
                </div>
                <UsageBar used={ing.used} limit={ing.limit ?? ing.used + 1} />
              </div>

              <div>
                <div className="flex items-center justify-between text-[12px]">
                  <span className="text-slate-600 dark:text-slate-300">Translations / SEO</span>
                  <span className="tabular-nums text-slate-500 dark:text-slate-400">
                    {fmtUsage(tr.used, tr.limit)}
                  </span>
                </div>
                <UsageBar used={tr.used} limit={tr.limit ?? tr.used + 1} />
              </div>
            </div>
          </div>
        </div>
