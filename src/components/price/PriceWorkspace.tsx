                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="text-[12px] font-semibold text-slate-900 dark:text-slate-50">{row.source_url ? row.source_url : shortId(row.id)}</div>
                      ...
                      <span className={cx("inline-flex items-center rounded-full border px-2 py-0.5 text-[12px] font-semibold", pill.cls)}>{pill.text}</span>
                    </div>

                    <div className="mt-2 flex flex-wrap items-center gap-3 text-[12px] text-slate-700 dark:text-slate-200">
                      ...
                      {pr?.warnings?.length ? <span className="rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[12px] font-semibold text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100">warnings {pr.warnings.length}</span> : null}
                    </div>
                  </button>
                );
              })}
          ...
          <div className="rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-[0_16px_40px_rgba(148,163,184,0.12)] dark:border-slate-800 dark:bg-slate-900/85" style={{ position: "sticky", top: 20 }}>
            ...
            <div className="mt-3 grid grid-cols-1 gap-3">
              <label className="space-y-1 text-xs">
                <div className="text-[12px] font-medium text-slate-600 dark:text-slate-300">Mode</div>
              ...
              <label className="space-y-1 text-xs">
                <div className="text-[12px] font-medium text-slate-600 dark:text-slate-300">Cost</div>
            ...
            <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-800 dark:bg-slate-950/40">
              <div className="flex items-center justify-between">
                <div className="text-[12px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Profile overrides</div>
              ...
              <div className="mt-3 grid grid-cols-1 gap-3">
                <label className="space-y-1 text-xs">
                  <div className="text-[12px] font-medium text-slate-600 dark:text-slate-300">Formula</div>
                ...
                <label className="space-y-1 text-xs">
                  <div className="text-[12px] font-medium text-slate-600 dark:text-slate-300">Value (decimal)</div>
                ...
                <label className="space-y-1 text-xs">
                  <div className="text-[12px] font-medium text-slate-600 dark:text-slate-300">Rounding</div>
            ...
            <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-800 dark:bg-slate-950/40">
              <div className="text-[12px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Latest compute</div>
              <div className="mt-2 grid grid-cols-2 gap-3 text-[12px]">
                <div>
                  <div className="text-[12px] text-slate-500 dark:text-slate-400">Computed</div>
                  <div className="font-semibold text-slate-900 dark:text-slate-50">{formatMoney(activeResult?.computedPrice)}</div>
                </div>
                <div>
                  <div className="text-[12px] text-slate-500 dark:text-slate-400">Store price</div>
                  <div className="font-semibold text-slate-900 dark:text-slate-50">{formatMoney(activeResult?.storePrice)}</div>
                </div>
              </div>

              {activeResult?.blocked ? <div className="mt-3 rounded-lg border border-rose-200 bg-rose-50 p-2 text-[12px] text-rose-900 dark:border-rose-500/25 dark:bg-rose-500/10 dark:text-rose-200">Blocked: {activeResult.blockReason ?? "unknown"}</div> : null}

              {activeExplain ? <details className="mt-3"><summary className="cursor-pointer text-[12px] font-semibold text-slate-700 dark:text-slate-200">Explain</summary><pre className="mt-2 overflow-auto rounded-lg border border-slate-200 bg-white p-2 text-[12px] text-slate-700 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200">{activeExplain}</pre></details> : <div className="mt-2 text-[12px] text-slate-500 dark:text-slate-400">Run compute to generate an explanation trail.</div>}
            </div>
          </div>
        ) : (
