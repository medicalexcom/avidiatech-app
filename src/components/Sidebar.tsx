...
export default function Sidebar({ variant = "desktop" }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      <div className="scrollbar-thin flex-1 space-y-4 overflow-y-auto overflow-x-hidden pr-0.5">
        {sections.map((section, sIdx) => (
          <div key={section.title}>
            ...
            <p className="mb-2 px-3 text-[11px] font-semibold uppercase tracking-[0.11em] text-slate-400 dark:text-slate-500 leading-none">
              {section.title}
            </p>
      ...
      <div className="mt-3 shrink-0 space-y-px border-t border-slate-200 dark:border-slate-800/80 pt-3">
        <Link
          href="/dashboard/assistant"
          className="flex items-center gap-3 rounded-lg px-3 py-2 text-[13px] font-medium leading-tight text-slate-500 transition-colors hover:bg-slate-100 dark:hover:bg-slate-800/50 hover:text-slate-700 dark:hover:text-slate-300"
