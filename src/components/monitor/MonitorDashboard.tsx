    ...
    <div className={cx("relative inline-flex items-center gap-2", className)}>
      ...
      {selected === "custom" ? (
        <div className="absolute left-0 top-full z-20 mt-2 w-[260px] rounded-xl border border-slate-200 bg-white p-3 shadow-lg">
          ...
          <div className="mt-2 text-[12px] text-slate-500">Enter a number of days, then Apply.</div>
        </div>
      ) : null}
    </div>
  );
...
function StatusBadge({ status }: { status?: string | null }) {
  ...
  return (
    <span className={cx("inline-flex items-center rounded-full border px-2 py-0.5 text-[12px] font-semibold capitalize", cls)}>
      {s.replace("_", " ")}
    </span>
  );
}
        ...
        <div className="col-span-2">
          ...
          {watches.length > WATCHES_PER_VIEW ? (
            <div className="mt-3 text-xs text-slate-500">
              Showing {WATCHES_PER_VIEW} of {watches.length} watches. <a className="underline" href="/dashboard/monitor">View all</a>
            </div>
          ) : null}
        </div>
