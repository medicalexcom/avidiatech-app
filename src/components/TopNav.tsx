...
/** AvidiaTech wordmark — indigo/violet gradient */
function BrandMark({ className }: { className?: string }) {
  return (
    <div className={`flex items-center justify-center rounded-xl bg-gradient-to-br from-indigo-600 to-violet-600 shadow-md shadow-indigo-600/20 ${className}`}>
      <svg viewBox="0 0 20 20" fill="none" className="h-[60%] w-[60%]" aria-hidden="true">
      ...
      <div className="flex h-[58px] w-full items-center justify-between gap-4 px-4 lg:px-5">
        ...
        <div className="flex shrink-0 items-center gap-2.5">
          ...
          <div className="hidden flex-col leading-none sm:flex">
            <span className="text-[12px] font-semibold uppercase tracking-[0.22em] text-slate-400 dark:text-slate-500">
              AvidiaTech
            </span>
          ...
          {/* Live indicator pill */}
          <span className="hidden items-center gap-1.5 rounded-full border border-slate-200/80 bg-slate-50 px-2.5 py-1 text-[12px] font-medium text-slate-500 sm:inline-flex dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
          ...
          {primaryLinks.map((link) => {
            ...
            return (
              <Link
                ...
                href={link.href}
                className={[
                  "relative inline-flex items-center rounded-lg px-3.5 py-1.5 text-[13px] font-medium transition-all duration-150",
                  active
        ...
        <div className="flex shrink-0 items-center gap-3">
          ...
          {isLoaded && isSignedIn ? (
            ...
            <button
              className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3.5 py-1.5 text-[13px] font-semibold text-white shadow-sm transition-all duration-150 hover:bg-indigo-700 hover:shadow disabled:opacity-50"
