...
/** AvidiaTech wordmark — tiny geometric A + T ligature */
      ...
      <div className="flex h-[58px] w-full items-center justify-between gap-4 px-4 lg:px-5">
          ...
          {primaryLinks.map((link) => {
            ...
            return (
              ...
              >
                {link.name}
                {active && (
                  <span className="pointer-events-none absolute -bottom-[9px] left-3 right-3 h-[2px] rounded-full bg-gradient-to-r from-cyan-400 via-sky-400 to-transparent" />
                )}
              </Link>
            );
          })}
        ...
        <div className="flex shrink-0 items-center gap-3">
          ...
          {isLoaded && isSignedIn ? (
            ...
            <button
              className="inline-flex items-center gap-1.5 rounded-lg bg-slate-900 px-3.5 py-1.5 text-[13px] font-semibold text-white shadow-sm transition-all duration-150 hover:bg-slate-800 hover:shadow disabled:opacity-50 dark:bg-cyan-500 dark:text-slate-950 dark:hover:bg-cyan-400"
