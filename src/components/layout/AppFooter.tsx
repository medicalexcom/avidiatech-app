      ...
      <div className="mx-auto flex h-11 max-w-screen-2xl items-center justify-between px-5 lg:px-6">
        ...
        <div className="flex items-center gap-2 text-[12px] text-slate-400 dark:text-slate-500">
          ...
          {version && (
            <>
              <span className="text-slate-300 dark:text-slate-700">·</span>
              <span className="rounded border border-slate-200 bg-slate-100 px-1.5 py-0.5 font-mono text-[12px] text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
                v{version}
              </span>
            </>
          )}
        ...
        <nav className="flex items-center gap-4" aria-label="Footer navigation">
          {footerLinks.map((link) => (
            <a
              ...
              className="text-[12px] text-slate-400 transition-colors hover:text-slate-700 dark:text-slate-500 dark:hover:text-slate-300"
            >
              {link.label}
            </a>
          ))}
