...
export async function processMatchUrlJob(jobId: string) {
  // load job header
  const { data: job } = await getServerSupabase().from("match_url_jobs").select("*").eq("id", jobId).maybeSingle();
  ...
  while (true) {
    ...
    const { data: rows } = await getServerSupabase()
    ...
    for (const r of rows) {
      try {
        await getServerSupabase().from("match_url_job_rows").update({ status: "running", updated_at: new Date().toISOString() }).eq("id", r.id);
        ...
        await getServerSupabase().from("match_url_job_rows").update(update).eq("id", r.id);
      } catch (err: any) {
        anyErrors = true;
        stats.errors++;
        await getServerSupabase().from("match_url_job_rows").update({ status: "error", error_message: String(err?.message ?? err), updated_at: new Date().toISOString() }).eq("id", r.id);
      }
    }
  ...
  await getServerSupabase().from("match_url_jobs").update({
    status: finalStatus,
