...
export default function Home() {
  ...
  const handleDemoRun = () => {
    ...
    // Mocked async behavior for now – easy to swap for a real API later.
    setTimeout(() => {
      setDemoOutput(
        `Sample SEO description for product at ${demoUrl}.\n\nAvidiaDescribe would generate a compliant, search-aware paragraph here based on your ingestion and custom GPT instructions. In your workspace, this becomes a full HTML + JSON payload ready to sync to your store.`
      );
      setDemoStatus("done");
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950 dark:bg-slate-950 dark:text-slate-50">
      <div className="mx-auto flex max-w-6xl flex-col gap-16 px-6 py-16 lg:py-20">
        ...
        <section className="grid items-center gap-12 lg:grid-cols-[1.1fr,0.9fr]">
          ...
          {/* Right side: hybrid “Try AvidiaDescribe” + workspace preview */}
...
We’ll simulate what AvidiaDescribe would return using your ingestion rules. In the real workspace, this becomes structured HTML + SEO JSON you can sync to your store or feeds.`}
        ...
        {/* PILLARS */}
        <section className="grid gap-6 rounded-3xl border border-slate-200 bg-white p-8 shadow-sm sm:grid-cols-3 dark:border-slate-800 dark:bg-slate-950">
          {pillars.map((item) => (
