  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-slate-200/80 bg-white/90 px-4 py-3 dark:border-slate-800 dark:bg-slate-900/80">
      ...
      <div className="min-w-0 flex-1">
        ...
        {sub && !loading && (
          <p className="mt-0.5 truncate text-[10px] text-slate-400 dark:text-slate-600">{sub}</p>
        )}
      </div>
    </div>
  );
  ...
  if (!items.length) {
    return (
      <div className="rounded-xl border border-dashed border-slate-200 py-8 text-center dark:border-slate-800">
        <p className="text-[11px] text-slate-400">No recent ingestions yet.</p>
        <Link href="/dashboard/extract" className="mt-2 inline-block text-[11px] font-medium text-indigo-600 hover:underline dark:text-indigo-400">
  ...
  return (
    <div className="space-y-1">
      {items.map((row: any) => {
        ...
        return (
          <div key={row.id}
            className="flex items-center justify-between gap-3 rounded-lg border border-slate-100 bg-slate-50/80 px-3 py-2 text-[11px] transition-colors hover:bg-white dark:border-slate-800/60 dark:bg-slate-900/50 dark:hover:bg-slate-900/80"
          >
...
export default function DashboardPage() {
  ...
  return (
    <div className="relative flex min-h-full w-full flex-col bg-slate-50 text-slate-900 dark:bg-[#060c1a] dark:text-slate-50">
        ...
        <section className="flex flex-col gap-6 lg:flex-row lg:items-start lg:gap-8">
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="inline-flex items-center gap-2 rounded-full border border-slate-200/80 bg-white/90 px-3 py-1.5 shadow-sm dark:border-slate-800 dark:bg-slate-900/80">
              ...
              <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Live</span>
              <span className="h-3 w-px bg-slate-200 dark:bg-slate-700" />
              <span className="text-[10px] text-slate-500 dark:text-slate-400">Product Data OS</span>
            </div>
            ...
            <div>
              ...
              <p className="mt-2 max-w-xl text-[13.5px] leading-relaxed text-slate-500 dark:text-slate-400">
                Extract, describe, optimize, and automate your entire product catalog — from a single URL to thousands of ready-to-publish pages.
              </p>
            </div>
          ...
          <div className="w-full lg:w-80 lg:shrink-0">
            <div className="rounded-2xl border border-slate-200/80 bg-white/95 p-4 shadow-card-md dark:border-slate-800 dark:bg-slate-900/80">
              <div className="mb-3 flex items-center justify-between">
                <span className="text-[10.5px] font-semibold uppercase tracking-[0.16em] text-slate-400 dark:text-slate-500">
              ...
              <div className="space-y-1.5">
                {[
                  { label: "Total ingestions",  value: metrics?.ingestions.total.toLocaleString() ?? "—", sub: metrics ? `${metrics.ingestions.success} ok · ${metrics.ingestions.failed} failed` : undefined, dot: "bg-emerald-400" },
                  { label: "Pipeline runs",      value: metrics?.pipeline.total_runs.toLocaleString() ?? "—", sub: metrics?.pipeline.success_rate != null ? `${metrics.pipeline.success_rate}% success` : undefined, dot: (metrics?.pipeline.running ?? 0) > 0 ? "bg-amber-400 animate-pulse" : "bg-cyan-400" },
                  { label: "SEO runs",           value: metrics?.ingestions.seo_runs.toLocaleString() ?? "—", sub: "via AvidiaSEO", dot: "bg-emerald-400" },
                  { label: "Describe runs",      value: metrics?.ingestions.describe_runs.toLocaleString() ?? "—", sub: "via AvidiaDescribe", dot: "bg-violet-400" },
                  ...
                  { label: "Monitor watches",    value: metrics?.monitor.watches.toLocaleString() ?? "—", sub: metrics ? `${metrics.monitor.events} events` : undefined, dot: "bg-rose-400" },
                ].map((row, i) => (
            ...
            {coreModules.map((mod) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Badge row */}
                  <div className="mb-3 flex items-center justify-between">
                    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-semibold ${mod.color.labelBg} ${mod.color.labelBorder} ${mod.color.label}`}>
                    ...
                    {group.items.map((item) => (
                      <li key={item.href}>
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <span className="hidden text-[10px] text-slate-400 dark:text-slate-500 sm:block">{item.tag}</span>
