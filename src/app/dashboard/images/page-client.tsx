  ...
  return (
    <div className={`rounded-xl border bg-white dark:bg-slate-900 ${image.approved ? "border-emerald-200/60 dark:border-emerald-500/30" : "border-slate-200 dark:border-slate-800"} ${dragging ? "opacity-50" : ""}`}>
      ...
      <div className={`relative h-28 w-full rounded-t-xl ${image.srcPlaceholder} flex items-center justify-center`}>
        <span className="text-[12px] font-medium text-slate-500 dark:text-slate-400">{image.width}×{image.height}</span>
        {image.approved && (
      ...
      <div className="p-2.5 space-y-2">
        <div className="flex items-start justify-between gap-1.5">
          <div>
            <p className="text-[12px] font-medium text-slate-800 dark:text-slate-200">{image.label}</p>
        ...
        {editingAlt ? (
          <div className="space-y-1">
            <textarea
              className="w-full rounded border border-violet-300 bg-white p-1.5 text-[12px] focus:outline-none focus:ring-1 focus:ring-violet-400 dark:border-violet-500/60 dark:bg-slate-800 dark:text-slate-100"
              rows={3}
              value={altDraft}
              onChange={(e) => setAltDraft(e.target.value)}
            />
            <div className="flex gap-1.5">
              <button onClick={() => { onEditAlt(image.id, altDraft); setEditingAlt(false); }} className="text-[12px] font-semibold text-emerald-600 hover:text-emerald-700">Save</button>
              <button onClick={() => { setAltDraft(image.altText); setEditingAlt(false); }} className="text-[12px] text-slate-400">Cancel</button>
            </div>
          </div>
        ) : (
          <button onClick={() => setEditingAlt(true)} className="group w-full text-left">
            <p className="text-[12px] text-slate-500 dark:text-slate-400 line-clamp-2 group-hover:text-violet-500">{image.altText}</p>
        ...
        <div className="flex gap-1.5 border-t border-slate-100 pt-1.5 dark:border-slate-800">
          <button
            ...
            className={`flex-1 rounded py-1 text-[12px] font-medium transition ${image.approved ? "bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300" : "bg-emerald-50 text-emerald-700 hover:bg-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-300"}`}
          >
      ...
      <>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white/95 p-4 sm:p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/85">
          ...
          <div className="space-y-1.5">
            <label className="block text-[12px] font-medium text-slate-700 dark:text-slate-200">Product URL</label>
            <input
              type="url"
          ...
          <div className="mt-4 flex gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg bg-rose-500 px-4 py-2 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(244,63,94,0.4)] transition hover:-translate-y-px hover:bg-rose-400 disabled:opacity-60"
            >
              {loading ? (
                <>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white/95 p-4 sm:p-5 shadow-[0_18px_45px_rgba(148,163,184,0.28)] dark:border-slate-800 dark:bg-slate-900/85">
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-[12px] text-slate-500 dark:text-slate-400">{imageSet.sourceUrl}</p>
        ...
        {!imageSet && !loading && (
          <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-slate-200 py-12 text-center dark:border-slate-800">
            ...
            <p className="text-[12px] text-slate-500">Select a sample product or paste a URL to extract and review images.</p>
          </div>
        )}
      </>
    </PageShell>
