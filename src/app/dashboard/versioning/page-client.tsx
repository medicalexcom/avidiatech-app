          ...
          <div className="w-full max-w-2xl rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900 mb-8">
            <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-800">
              <div>
                ...
                <p className="text-[12px] text-slate-500 dark:text-slate-400">{diffEntry.productName} · {diffEntry.sku}</p>
              </div>
            ...
            <div className="px-5 py-4 space-y-1">
              <div className="grid grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_minmax(0,1fr)] gap-2 pb-2 text-[12px] font-semibold uppercase tracking-[0.14em] text-slate-400 dark:text-slate-600 px-3">
                <span>Field</span>
                <span>Before</span>
                <span>After</span>
              </div>
              ...
              <div className="flex gap-1 flex-wrap">
                {(["All", "Specs", "SEO", "Description", "Price"] as const).map((f) => (
                  <button
                    ...
                    className={`rounded-full px-2.5 py-0.5 text-[12px] font-semibold uppercase transition-colors ${
                      filterField === f
                    ...
                    <div key={entry.id} className="flex items-start gap-3 px-4 py-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className={`rounded-full border px-1.5 py-0.5 text-[12px] font-semibold uppercase ${FIELD_COLORS[entry.fieldType]}`}>
                            {entry.fieldType}
                          </span>
                          <span className={`text-[12px] font-medium ${SOURCE_COLORS[entry.source]}`}>
                            by {entry.author}
                          </span>
                          <span className="text-[12px] text-slate-400 dark:text-slate-600">{fmtDate(entry.createdAt)}</span>
                        </div>
                        <p className="mt-1 text-xs text-slate-600 dark:text-slate-300">{entry.summary}</p>
                        <div className="mt-2 flex gap-2">
                          <button
                            onClick={() => setDiffEntry(entry)}
                            className="rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-[12px] font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
                          >
                            View diff
                          </button>
                          <button
                            ...
                            className="rounded-lg bg-emerald-500 px-2.5 py-1 text-[12px] font-semibold text-white hover:bg-emerald-400 disabled:opacity-60"
                          >
