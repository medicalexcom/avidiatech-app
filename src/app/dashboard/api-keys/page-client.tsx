...
function ScopeBadge({ scope }: { scope: string }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[12px] font-semibold uppercase tracking-wide ${colors[scope] ?? "bg-slate-100 border-slate-200 text-slate-600 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-400"}`}>
      {scope}
    </span>
  );
}
                ...
                <div className="flex flex-wrap gap-2">
                  {SCOPES.map((scope) => (
                    <button
                      ...
                      className={`rounded-full border px-3 py-1 text-[12px] font-semibold uppercase transition-colors ${
              ...
              {activeKeys.map((key) => (
                <div key={key.id} className="flex flex-wrap items-center gap-3 px-5 py-3.5">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-0.5 flex flex-wrap items-center gap-3 text-[12px] text-slate-400 dark:text-slate-600">
            ...
            <div className="divide-y divide-slate-100/60 dark:divide-slate-800/40">
              {revokedKeys.map((key) => (
                <div key={key.id} className="flex flex-wrap items-center gap-3 px-5 py-3 opacity-50">
                  ...
                  <span className="rounded-full border border-rose-200 bg-rose-50 px-2 py-0.5 text-[12px] font-semibold uppercase text-rose-600 dark:border-rose-500/20 dark:bg-rose-950/20 dark:text-rose-400">
                    Revoked
                  </span>
                </div>
              ))}
