...
import IntegrationsModal from "@/components/connectors/IntegrationsModal";
...
export default function ImportPage() {
  ...
  // modal to manage connections (replaces long inline list)
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-50">
              ...
              <div className="rounded-3xl border border-slate-200 bg-gradient-to-b from-white/95 to-slate-50/70 p-4 shadow-sm dark:border-slate-800 dark:from-slate-950/35">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/70 px-3 py-1 text-[11px] text-slate-600 shadow-sm dark:border-slate-800 dark:bg-slate-950/35">
                      <span className="font-mono">{connectors.filter(c => (c.status ?? "active") === "active").length}</span> stores
                    </span>
                  </div>
                </div>

                <div className="mt-4">
                  {/* Compact controls: Manage opens modal, Add store goes to the connect flow */}
                  <div className="flex gap-2">
                    ...
                    <a href="/integrations/new" className="px-3 py-2 rounded-lg bg-emerald-600 text-white text-sm">
                      Add store
                    </a>
                  </div>

                  <div className="mt-4 rounded-2xl border border-slate-200/60 bg-white/50 p-3 backdrop-blur dark:border-slate-800/70 dark:bg-slate-950/30">
                    {/* Selected store summary and quick actions */}
                    ...
                    {selectedConnectorObj ? (
                      ...
                      <div className="mt-2 text-xs text-slate-600 dark:text-slate-300">Select a store to enable actions. Use Manage connections or Add store to change connectors.</div>
                    )}
                  </div>
                </div>
      ...
      {/* Integrations modal (manage connections) */}
