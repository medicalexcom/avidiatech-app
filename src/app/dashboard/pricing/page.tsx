...
export default function PricingPage() {
  ...
  // Check if user already has an active subscription (to show upgrade vs new checkout)
  const [hasSubscription, setHasSubscription] = useState(false);
  React.useEffect(() => {
    if (!isLoaded || !isSignedIn) return;
    fetch("/api/subscription/status")
      .then((r) => r.json())
      .then((d) => { if (d?.active && !d?.isOwner) setHasSubscription(true); })
      .catch(() => {});
  }, [isLoaded, isSignedIn]);
  ...
  async function choosePlan(plan: "starter" | "growth" | "pro") {
    ...
    try {
      if (hasSubscription) {
        ...
        setSuccess(`You are now on the ${plan.charAt(0).toUpperCase() + plan.slice(1)} plan. Changes take effect immediately.`);
    ...
    <PageShell glow="indigo">
      ...
      <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2.5 rounded-2xl border border-slate-200/60 bg-slate-50/60 px-5 py-4 dark:border-slate-800/60 dark:bg-slate-900/50">
        ...
        <div className="flex items-center gap-3 text-[11.5px]">
          <a href="/privacy" className="text-slate-400 underline-offset-2 transition hover:text-slate-600 hover:underline dark:text-slate-500 dark:hover:text-slate-300">Privacy</a>
          <a href="/terms" className="text-slate-400 underline-offset-2 transition hover:text-slate-600 hover:underline dark:text-slate-500 dark:hover:text-slate-300">Terms</a>
        </div>
      </div>
    </PageShell>
  );
