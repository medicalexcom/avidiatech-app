      ...
      <div className="relative mx-auto max-w-7xl space-y-6 px-4 pt-4 pb-8 sm:px-6 lg:px-8 lg:pt-6 lg:pb-10">
        ...
        <section
          ...
          className={cx(
            ...
            "shadow-[0_18px_55px_-40px_rgba(2,6,23,0.45)] backdrop-blur-xl",
        ...
        >
          ...
          {/* ✅ No extra framed wrapper here — just spacing */}
          <div className="mt-4">
            <DescribeForm />
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-12 lg:gap-8">
          {/* PREVIEW (single surface, no nested frames) */}
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    HTML is the default view. Switch to JSON only for debugging.
                  </p>
                </div>
              </div>
          ...
          {/* RIGHT RAIL (kept clean; already single-frame cards) */}
          <aside className="space-y-4 lg:col-span-4">
            <div className="rounded-2xl border border-slate-200/70 bg-white/75 p-4 shadow-sm backdrop-blur dark:border-slate-800/60 dark:bg-slate-950/45">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    Fast, consistent, usable downstream.
                  </p>
                </div>
                <TinyChip tone="brand">
                  <span className="h-1.5 w-1.5 rounded-full bg-fuchsia-500 dark:bg-fuchsia-300" />
              ...
              <div className="mt-4 space-y-2 text-[11px]">
                {[
                  ["1", "Fill the input block", "Name + short context + constraints you care about."],
                  ["2", "Review the canvas", "Claims, disclaimers, and structure match your rules."],
                  ["3", "Send downstream", "Reuse structure in SEO, Import, exports."],
                ].map(([n, t, b]) => (
                  <div
                    key={n}
                  ...
                  >
                    <div className="mt-[1px] flex h-6 w-6 items-center justify-center rounded-lg border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
                      <span className="text-[12px] font-semibold text-slate-700 dark:text-slate-200">
                        {n}
                      </span>
                    </div>
                    <div className="min-w-0">
                      <div className="font-semibold text-slate-900 dark:text-slate-50">{t}</div>
                      <div className="mt-0.5 text-slate-600 dark:text-slate-300">{b}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-2xl border border-slate-200/70 bg-white/75 p-4 shadow-sm backdrop-blur dark:border-slate-800/60 dark:bg-slate-950/45">
              ...
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                Notes → instructioned output → HTML-first preview.
              </p>

              <div className="mt-4 space-y-2 text-[11px]">
                {[
                  { dot: "bg-fuchsia-400", title: "Input channel", body: "Notes, specs, constraints." },
                  { dot: "bg-sky-400", title: "Instruction profile", body: "Tone, sections, meta, disclaimers." },
                  { dot: "bg-emerald-400", title: "Output canvas", body: "HTML (default) + payload." },
                ].map((x) => (
                  <div
                    key={x.title}
