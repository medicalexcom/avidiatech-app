...
function StatusBadge({ status }: { status: DescribeStatus }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[12px] font-semibold uppercase tracking-wide ${map[status]}`}>
      {status === "running" && (
  ...
  return (
    <div className="relative flex min-h-full w-full flex-col bg-slate-50 dark:bg-slate-950 dark:text-slate-50 overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -top-24 -left-16 h-64 w-64 rounded-full bg-violet-400/28 blur-3xl dark:bg-violet-500/20" />
      ...
      <div className="relative mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-10 lg:py-8">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
          {/* Header */}
          <div className="grid grid-cols-[minmax(0,2.5fr)_auto_auto_auto_auto_auto_auto] items-center border-b border-slate-200 bg-slate-50/80 px-4 py-2.5 text-[12px] font-semibold uppercase tracking-[0.16em] text-slate-500 dark:border-slate-800 dark:bg-slate-950/60 dark:text-slate-400 gap-4">
            <span>Product</span>
            <span>Status</span>
            <span>Tone</span>
            <span>Words</span>
                ...
                <div key={entry.id}>
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-[12px] text-slate-400 dark:text-slate-500">{entry.url}</p>
                    </div>
                    <StatusBadge status={entry.status} />
                    <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[12px] text-slate-600 dark:bg-slate-800 dark:text-slate-300 whitespace-nowrap">{entry.tone}</span>
                    ...
                    <span className="text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{fmtDate(entry.created_at)}</span>
                  </div>
                    ...
                    <div className="border-t border-slate-100 bg-slate-50/60 px-4 py-3 dark:border-slate-800/60 dark:bg-slate-950/40">
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="flex-1 space-y-1">
                          <p className="text-[12px] font-semibold text-slate-500 dark:text-slate-400">Ingestion ID</p>
                          <p className="font-mono text-xs text-slate-700 dark:text-slate-200">{entry.ingestion_id}</p>
                        </div>
                        <div className="flex-1 space-y-1">
                          <p className="text-[12px] font-semibold text-slate-500 dark:text-slate-400">Channel</p>
