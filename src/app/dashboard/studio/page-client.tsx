      ...
      <div className={`space-y-4 px-4 py-4 ${isMarketplace ? "px-3 py-3" : ""}`}>
        {blocks.includes("Hero + primary CTA") && (
          <div className={`${isMarketplace ? "" : "grid grid-cols-1 gap-4 md:grid-cols-[minmax(0,1.2fr),minmax(0,0.9fr)]"}`}>
            ...
            {!isMarketplace && !isMobile && (
              <div className="h-28 rounded-lg border border-slate-200 bg-gradient-to-br from-slate-100 to-slate-50 dark:border-slate-700 dark:from-slate-800 dark:to-slate-900 flex items-center justify-center text-[12px] text-slate-400">
                Product imagery
              </div>
            )}
          </div>
        ...
        {blocks.includes("Key features grid") && (
          <div className="border-t border-slate-100 pt-3 dark:border-slate-800">
            <h4 className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-300">Key features</h4>
        ...
        {blocks.includes("Specifications table") && (
          <div className="border-t border-slate-100 pt-3 dark:border-slate-800">
            <h4 className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-300">Specifications</h4>
            <div className="divide-y divide-slate-100 rounded-lg border border-slate-200 dark:divide-slate-800 dark:border-slate-700">
              {sampleProduct.specs.slice(0, template === "visual" ? 3 : 5).map((s) => (
                <div key={s.key} className="flex items-center justify-between px-3 py-1.5 text-[12px]">
        ...
        {blocks.includes("Downloads & manuals") && (
          <div className="border-t border-slate-100 pt-3 dark:border-slate-800">
            <h4 className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-300">Manuals & downloads</h4>
        ...
        {blocks.includes("Usage & care") && (
          <div className="border-t border-slate-100 pt-3 dark:border-slate-800">
            <h4 className="mb-1.5 text-[12px] font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-300">Usage & care</h4>
            <p className="text-[12px] text-slate-600 dark:text-slate-300">Clean with hospital-grade quaternary ammonium wipes. Do not submerge. Inspect casters quarterly.</p>
          </div>
        )}

        {blocks.includes("Warranty & returns") && (
          <div className="border-t border-slate-100 pt-3 dark:border-slate-800">
            <h4 className="mb-1.5 text-[12px] font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-300">Warranty & returns</h4>
            <p className="text-[12px] text-slate-600 dark:text-slate-300">1-year manufacturer warranty. 30-day returns accepted for unused items in original packaging.</p>
          </div>
        )}

        {blocks.includes("FAQs / Q&A") && (
          <div className="border-t border-slate-100 pt-3 dark:border-slate-800">
            <h4 className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-300">FAQs</h4>
            <div className="space-y-2">
              {sampleProduct.faqs.slice(0, 2).map((faq) => (
                <div key={faq.q} className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 dark:border-slate-800 dark:bg-slate-950/80">
                  <p className="text-[12px] font-medium text-slate-800 dark:text-slate-200">{faq.q}</p>
                  <p className="mt-0.5 text-[12px] text-slate-500 dark:text-slate-400">{faq.a}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
            <div className="min-w-[260px] flex-1 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                <div className="inline-flex items-center gap-2 rounded-full border border-fuchsia-500/60 bg-white/90 px-3 py-1.5 text-[12px] font-medium uppercase tracking-[0.18em] text-slate-500 shadow-sm dark:bg-slate-950/90 dark:text-slate-300">
            ...
            <div className="w-full lg:w-[340px]">
              <div className="space-y-2 rounded-2xl border border-slate-200 bg-white/95 px-4 py-4 shadow-sm dark:border-slate-800/80 dark:bg-slate-950/95">
                <p className="text-[12px] font-medium uppercase tracking-[0.2em] text-slate-400">Template presets</p>
                {(Object.entries(templatePresets) as [Template, typeof templatePresets[Template]][]).map(([key, tpl]) => (
                  ...
                  >
                    <p className="text-[12px] font-semibold text-slate-900 dark:text-slate-100">{tpl.name}</p>
                    <p className="mt-0.5 text-[12px] text-slate-500 dark:text-slate-400">{tpl.desc}</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <label className="block text-[12px] font-medium text-slate-800 dark:text-slate-200">Brand / storefront</label>
                  <select
                ...
                <div className="space-y-1.5">
                  <label className="block text-[12px] font-medium text-slate-800 dark:text-slate-200">Target channel</label>
                  <select
                    className="w-full rounded-lg border border-slate-300 bg-white/90 px-3 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-fuchsia-500/40 dark:border-slate-700 dark:bg-slate-950/80 dark:text-slate-100"
              ...
              {/* Page blocks */}
              <div className="space-y-2">
                <label className="block text-[12px] font-medium text-slate-800 dark:text-slate-200">Page blocks</label>
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {ALL_BLOCKS.map((b) => (
                    ...
                    >
                      ...
                      />
                      <span className="text-[12px] text-slate-700 dark:text-slate-200">{b}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <label className="block text-[12px] font-medium text-slate-800 dark:text-slate-200">Primary CTA</label>
                  <select
                ...
                <div className="space-y-1.5">
                  <label className="block text-[12px] font-medium text-slate-800 dark:text-slate-200">Secondary CTA</label>
                  <select
                    className="w-full rounded-lg border border-slate-300 bg-white/90 px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-cyan-500/40 dark:border-slate-700 dark:bg-slate-950/80 dark:text-slate-100"
