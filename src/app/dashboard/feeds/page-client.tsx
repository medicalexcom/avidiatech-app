...
function CoverageBar({ normalized, total }: { normalized: number; total: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[12px] font-semibold tabular-nums" style={{ color }}>{Math.round(pct)}%</span>
    </div>
  );
}
      ...
      {/* HEADER */}
        <section className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-lg bg-orange-500 px-4 py-2 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(249,115,22,0.4)] transition hover:-translate-y-px hover:bg-orange-400"
          >
        ...
        {/* Add feed form */}
        {showAdd && (
          <div className="rounded-2xl border border-orange-200/60 bg-orange-50/70 p-4 sm:p-5 dark:border-orange-500/30 dark:bg-orange-500/5">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <input
                ...
                className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs focus:border-orange-400 focus:outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
              />
              <input
            ...
            <div className="mt-3 flex gap-2">
              <button onClick={addFeed} disabled={!newName.trim() || !newSource.trim()} className="rounded-lg bg-orange-500 px-4 py-1.5 text-xs font-semibold text-white transition hover:bg-orange-400 disabled:opacity-60">
                Add feed
              </button>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white/95 shadow-[0_18px_45px_rgba(148,163,184,0.35)] dark:border-slate-800 dark:bg-slate-900/85 dark:shadow-[0_18px_45px_rgba(15,23,42,0.7)] overflow-hidden">
          <div className="grid grid-cols-[2fr_1fr_1fr_1fr_auto] gap-x-4 border-b border-slate-100 bg-slate-50 px-5 py-2.5 text-[12px] font-medium uppercase tracking-wide text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
            <span>Feed</span>
          ...
          {feeds.map((feed) => (
            <div
              key={feed.id}
              className={`grid grid-cols-[2fr_1fr_1fr_1fr_auto] gap-x-4 border-b border-slate-50 px-5 py-3.5 text-[12px] last:border-0 hover:bg-slate-50/80 dark:border-slate-900 dark:hover:bg-slate-950/50 ${selectedFeed === feed.id ? "bg-sky-50/50 dark:bg-sky-500/5" : ""}`}
            >
              <div>
                <p className="font-semibold text-slate-900 dark:text-slate-50">{feed.name}</p>
                <p className="text-[12px] text-slate-400">{feed.source} · {feed.format} · Last: {feed.lastRun}</p>
              </div>
              ...
              <div className="self-center">
                ...
                <p className="mt-0.5 text-[12px] text-slate-400">{feed.normalizedRows.toLocaleString()}/{feed.totalRows.toLocaleString()} rows</p>
              </div>
              <div className="self-center">
                <StatusBadge status={feed.status} />
                {feed.missingFields > 0 && (
                  <p className="mt-0.5 text-[12px] text-rose-500">{feed.missingFields} missing fields</p>
                )}
              </div>
            ...
            <div className="space-y-2">
              {DESTINATIONS.slice(0, 4).map((dest) => (
                <div key={dest} className="flex items-center justify-between rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 dark:border-slate-800 dark:bg-slate-950/70">
                  <span className="text-[12px] text-slate-700 dark:text-slate-200">{dest}</span>
                  <button
                    onClick={() => showToast(`Export to "${dest}" queued.`)}
                    className="rounded-full border border-slate-200 bg-white px-2.5 py-0.5 text-[12px] font-medium text-slate-600 transition hover:border-emerald-300 hover:text-emerald-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"
                  >
                    Export →
                  </button>
                </div>
