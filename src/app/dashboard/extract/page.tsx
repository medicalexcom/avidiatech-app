...
/**
 * Dashboard Extract page — refactored to reuse ExtractHeader and surface highlights.
 *
 * - Reuses src/components/ExtractHeader.tsx to submit ingests and get jobId via onJobCreated.
 * - Shows highlights (name, features, pdf_manual_urls, images, quality_score) at the top of the left pane
 *   so product managers / merch can validate immediately.
 * - Keeps existing TabsShell + JsonViewer on the page.
 *
...
export default function ExtractPage() {
  ...
  const [submitting, setSubmitting] = useState(false); // still useful if you want to show global state
  const [jobId, setJobId] = useState<string | null>(null);
  const { row, loading: rowLoading, error: rowError } = useIngestRow(jobId, 1500);

  // derive the normalized payload to read highlights from
  const payload = useMemo(() => {
    return (row?.normalized_payload ?? row) || null;
  }, [row]);

  // small helper to get highlights
  ...
  // invoked by ExtractHeader when it receives a job id from the API
  function onJobCreated(id: string) {
    setJobId(String(id));
    // scroll to results area shortly after job is created
    setTimeout(() => {
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Top header: use existing ExtractHeader component (it posts /api/v1/ingest and calls onJobCreated) */}
      <header>
      ...
      <main className="grid grid-cols-1 lg:grid-cols-[1fr,420px] gap-6">
        {/* Left pane: human readable + highlights */}
        <section className="bg-white rounded-lg shadow-sm p-4 min-h-[520px]">
                ...
                <div className="flex gap-4">
                  <div style={{ flex: 1 }}>
                    ...
                    <div className="mt-2 text-sm text-slate-700">
                      {quality !== undefined && (
                        <span className="mr-3">Quality: <strong>{quality}</strong></span>
                      )}
                      {needsReview !== undefined && (
                        <span className="mr-3">Needs review: <strong>{String(Boolean(needsReview))}</strong></span>
                      )}
                    </div>
          ...
          {/* TabsShell remains the canonical human readable component */}
          <div className="mt-4 id-extract-results" id="extract-results">
