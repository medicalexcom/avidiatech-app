    ...
    <div className="rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950/80">
      ...
      >
        <div className="flex items-center gap-2.5">
          <span className="inline-flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full border border-violet-400/60 bg-violet-500/10 text-[11px] font-semibold text-violet-600 dark:bg-violet-500/15 dark:text-violet-100">
            {cluster.cluster_id}
          </span>
          <div>
            <p className="text-[11px] font-semibold text-slate-900 dark:text-slate-50">{cluster.label}</p>
        ...
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 text-[10px] text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
            Similarity
          </span>
          <svg
      ...
      <div className="flex flex-wrap items-center gap-1.5 border-t border-slate-100 px-3.5 py-2 dark:border-slate-800">
        <span className="text-[10px] text-slate-400">Top signals:</span>
        {cluster.topSignals.map((sig) => (
          <span key={sig} className="rounded-full border border-slate-200 bg-slate-100 px-2 py-[2px] text-[10px] text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
            {sig}
          </span>
        ))}
      </div>
      ...
      {expanded && (
        <div className="border-t border-slate-100 px-3.5 pb-3 pt-2.5 dark:border-slate-800">
          <p className="mb-2 text-[10px] font-medium uppercase tracking-[0.15em] text-slate-500">Members (sample)</p>
          <div className="space-y-1.5">
            {cluster.members.map((m) => (
              <div key={m.product_id} className="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[11px] dark:border-slate-700 dark:bg-slate-900">
                <div>
                ...
                <span className="text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">{m.similarity.toFixed(2)}</span>
              </div>
            ))}
          </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <button
              ...
              className="rounded-full border border-sky-400/60 bg-sky-500/10 px-2.5 py-1 text-[10px] font-medium text-sky-700 transition hover:bg-sky-500/15 dark:border-sky-400/60 dark:text-sky-200"
            >
              Send to AvidiaMatch
            </button>
            <button
              onClick={() => onAction("variants", cluster.cluster_id)}
              className="rounded-full border border-emerald-400/60 bg-emerald-500/10 px-2.5 py-1 text-[10px] font-medium text-emerald-700 transition hover:bg-emerald-500/15 dark:border-emerald-400/60 dark:text-emerald-200"
            >
              Build variants
            </button>
            <button
              onClick={() => onAction("csv", cluster.cluster_id)}
              className="rounded-full border border-slate-300 bg-white px-2.5 py-1 text-[10px] font-medium text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
            >
              Export CSV
            </button>
          </div>
            ...
            {clusters && (
              <div className="w-full lg:w-[340px]">
                <div className="space-y-3 rounded-2xl border border-slate-200 bg-white/95 px-4 py-4 shadow-sm dark:border-slate-800/80 dark:bg-slate-950/95">
                  <p className="text-[11px] font-medium uppercase tracking-[0.2em] text-slate-400">Run results</p>
        ...
        <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.05fr,1.1fr] lg:gap-6">
          ...
          <div className="space-y-4 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-[0_18px_45px_rgba(148,163,184,0.35)] lg:p-5 dark:border-slate-800/80 dark:bg-slate-900/90 dark:shadow-[0_18px_45px_rgba(15,23,42,0.7)]">
            ...
            >
              {/* Source scope */}
              <div className="space-y-1.5">
                <label className="block text-[11px] font-medium text-slate-800 dark:text-slate-200">Source scope</label>
                <select
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <label className="block text-[11px] font-medium text-slate-800 dark:text-slate-200">
                    ...
                    <span className="ml-1 text-[10px] font-normal text-violet-600 dark:text-violet-300">{threshold.toFixed(2)}</span>
                  </label>
                  <input
                ...
                <div className="space-y-1.5">
                  <label className="block text-[11px] font-medium text-slate-800 dark:text-slate-200">Minimum cluster size</label>
                  <select
                    className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-900 shadow-sm focus:border-violet-400 focus:outline-none focus:ring-2 focus:ring-violet-500/40 dark:border-slate-700 dark:bg-slate-950/80 dark:text-slate-100"
                    value={minSize}
              ...
              {/* Fields */}
              <div className="space-y-2">
                <label className="block text-[11px] font-medium text-slate-800 dark:text-slate-200">Fields to prioritize</label>
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {fieldOptions.map((f) => (
                    ...
                    >
                      ...
                      />
                      <span className="text-[11px] text-slate-700 dark:text-slate-200">{f}</span>
                    </label>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-4 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-[0_18px_45px_rgba(148,163,184,0.35)] lg:p-5 dark:border-slate-800/80 dark:bg-slate-900/90 dark:shadow-[0_18px_45px_rgba(15,23,42,0.7)]">
            ...
            {!running && !clusters && (
              <div className="flex flex-col items-center gap-3 rounded-xl border border-dashed border-slate-200 py-10 text-center dark:border-slate-800">
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400">Configure parameters and click "Run clustering."</p>
              </div>
            )}
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3 dark:border-slate-800 dark:bg-slate-950/80">
                  <div className="flex items-center justify-between text-[11px] text-slate-700 dark:text-slate-300">
                    ...
                    <button
                      className="text-[10px] text-violet-500 underline underline-offset-2 hover:text-violet-600"
                      onClick={() => {
                  ...
                  <pre className="mt-2 max-h-40 overflow-auto rounded-lg border border-slate-200 bg-white p-2 text-[10px] text-slate-700 dark:border-slate-900 dark:bg-slate-950/90 dark:text-slate-300">
