        ...
        <div className="border-t border-slate-100 px-4 pb-4 pt-3 dark:border-slate-800">
          ...
          <div className="mb-3 space-y-1.5">
            {family.options.map((opt) => (
              <div key={opt.name} className="flex flex-wrap items-center gap-2 text-[12px]">
          ...
          <div className="rounded-lg border border-slate-200 dark:border-slate-800 overflow-hidden">
            <div className="grid grid-cols-[1fr_auto] gap-x-4 border-b border-slate-100 bg-slate-100 px-3 py-1.5 text-[12px] font-medium uppercase tracking-wide text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
              <span>Variant</span>
              <span>SKU</span>
            </div>
            {family.members.map((m) => (
              <div key={m.sku} className="grid grid-cols-[1fr_auto] gap-x-4 border-b border-slate-100 px-3 py-2 text-[12px] last:border-0 dark:border-slate-800">
                <div>
                  <p className="font-medium text-slate-800 dark:text-slate-200">{m.name}</p>
                  <p className="text-[12px] text-slate-400">{Object.entries(m.attrs).map(([k, v]) => `${k}: ${v}`).join(" · ")}</p>
                </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {!family.approved && (
              <button onClick={() => onApprove(family.id)} className="rounded-full border border-emerald-400/60 bg-emerald-500/10 px-3 py-1 text-[12px] font-medium text-emerald-700 transition hover:bg-emerald-500/20 dark:text-emerald-300">
                ✓ Approve family
              </button>
            )}
            <button onClick={() => onReject(family.id)} className="rounded-full border border-rose-400/60 bg-rose-50 px-3 py-1 text-[12px] font-medium text-rose-700 transition hover:bg-rose-100 dark:border-rose-500/40 dark:bg-rose-500/10 dark:text-rose-300">
              Discard
            </button>
            <button onClick={() => onExport(family.id)} className="rounded-full border border-slate-300 bg-white px-3 py-1 text-[12px] font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
              Export CSV
            </button>
            <button className="rounded-full border border-sky-400/60 bg-sky-500/10 px-3 py-1 text-[12px] font-medium text-sky-700 transition hover:bg-sky-500/20 dark:text-sky-300">
              Sync to store
            </button>
          </div>
        </div>
      ...
      <>
        ...
        <section className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex-1 space-y-3">
            <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/90 px-3 py-1.5 text-[12px] font-medium uppercase tracking-[0.18em] text-slate-700 shadow-sm dark:border-slate-800 dark:bg-slate-950/90 dark:text-slate-300">
          ...
          <div className="flex flex-wrap gap-3">
            {[
              ...
              <div key={s.label} className="rounded-xl border border-slate-200 bg-white/95 px-4 py-2.5 shadow-sm dark:border-slate-800 dark:bg-slate-950/90 text-center min-w-[72px]">
                ...
                <p className="text-[12px] text-slate-500 dark:text-slate-400">{s.label}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700">
            {(["all", "pending", "approved"] as const).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-3 py-1.5 text-[12px] font-medium capitalize transition ${
                  filter === f
          ...
          <button
            ...
            className="rounded-lg border border-emerald-300/60 bg-emerald-50 px-3 py-1.5 text-[12px] font-medium text-emerald-700 transition hover:bg-emerald-100 dark:border-emerald-500/40 dark:bg-emerald-500/10 dark:text-emerald-300"
          >
            Approve all
          </button>
        </div>
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-3 rounded-xl border border-dashed border-slate-200 py-10 text-center dark:border-slate-800">
              ...
              <p className="text-[12px] text-slate-500">Click "Run variant detection" to propose families from your catalog.</p>
            </div>
          ) : (
            displayed.map((f) => (
              <FamilyCard
