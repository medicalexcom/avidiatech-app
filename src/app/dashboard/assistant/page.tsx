...
export default function AssistantPage() {
  ...
  return (
    <div className="relative flex flex-col flex-1 min-h-0 bg-slate-50 dark:bg-slate-950 overflow-hidden">
      {/* Ambient: cyan-emerald wash for AI assistant identity */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute left-0 top-0 h-[3px] w-full" style={{ backgroundImage: "linear-gradient(90deg,#06b6d4 0%,#10b981 60%,transparent 100%)" }} />
        <div className="absolute left-0 top-[3px] h-[60%] w-full dark:hidden" style={{ backgroundImage: "linear-gradient(180deg,rgba(6,182,212,0.07) 0%,rgba(16,185,129,0.03) 38%,transparent 68%)" }} />
        <div className="absolute left-0 top-[3px] h-[60%] w-full hidden dark:block" style={{ backgroundImage: "linear-gradient(180deg,rgba(6,182,212,0.18) 0%,rgba(16,185,129,0.07) 34%,transparent 62%)" }} />
        <div className="absolute -top-20 -left-20 h-64 w-64 rounded-full bg-cyan-300/15 blur-3xl dark:bg-cyan-500/12" />
        <div className="absolute -bottom-20 -right-10 h-56 w-56 rounded-full bg-emerald-300/12 blur-3xl dark:bg-emerald-500/10" />
      </div>
      {/* Header */}
      <div className="flex-none px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="h-9 w-9 rounded-full bg-gradient-to-br from-cyan-500 to-emerald-500 flex items-center justify-center text-white font-bold text-sm">AI</div>
          <div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
            ...
            <div className={`max-w-[75%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
              ...
              <p className={`text-[10px] mt-1.5 ${msg.role === "user" ? "text-cyan-200" : "text-slate-400"}`}>{fmtTime(msg.ts)}</p>
            </div>
          </div>
        ))}
        ...
        {loading && (
          <div className="flex gap-3">
            <div className="flex-none h-8 w-8 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900 dark:text-emerald-300 flex items-center justify-center text-xs font-semibold">AI</div>
          ...
          <div className="flex flex-wrap gap-2">
            {SUGGESTIONS.map((s) => (
              <button key={s} onClick={() => send(s)}
                className="text-xs px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:border-cyan-400 hover:text-cyan-700 dark:hover:text-cyan-400 transition-colors">
                {s}
              </button>
            ))}
          </div>
      ...
      <div className="flex-none px-4 pb-4 pt-2 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
        <form onSubmit={handleSubmit} className="flex gap-2 items-end">
          <textarea
            ...
            rows={2}
            disabled={loading}
            className="flex-1 resize-none rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-transparent disabled:opacity-50 transition-colors"
          />
          <button type="submit" disabled={!input.trim() || loading}
            className="flex-none h-11 w-11 rounded-xl bg-cyan-600 hover:bg-cyan-700 disabled:bg-slate-200 dark:disabled:bg-slate-700 disabled:cursor-not-allowed text-white flex items-center justify-center transition-colors shadow-sm"
        ...
        <p className="text-[10px] text-slate-400 mt-1.5 px-1">Shift+Enter for new line · responses are AI-generated</p>
      </div>
    </div>
  );
}
