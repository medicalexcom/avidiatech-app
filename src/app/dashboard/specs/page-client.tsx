  ...
  return (
    <PageShell glow="teal">
      ...
      <>
        {/* HEADER */}
        <section>
          <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/90 px-3 py-1 text-[12px] font-medium uppercase tracking-[0.18em] text-slate-500 shadow-sm dark:border-slate-800 dark:bg-slate-950/90 dark:text-slate-300 mb-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white/95 p-4 sm:p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/85">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg bg-teal-500 px-4 py-2 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(20,184,166,0.4)] transition hover:-translate-y-px hover:bg-teal-400 disabled:opacity-60"
            >
              {running ? (
                <>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white/95 p-4 sm:p-5 shadow-[0_18px_45px_rgba(148,163,184,0.28)] dark:border-slate-800 dark:bg-slate-900/85">
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-[12px] text-slate-500 dark:text-slate-400">{result.source}</p>
              </div>
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800">
              <div className="grid grid-cols-[1fr_1fr_1fr_auto_auto] gap-x-3 border-b border-slate-100 bg-slate-50 px-4 py-2 text-[12px] font-medium uppercase tracking-wide text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
                <span>Raw key</span>
                <span>Raw value</span>
              ...
              {rows.map((row) => (
                <div key={row.rawKey} className="grid grid-cols-[1fr_1fr_1fr_auto_auto] gap-x-3 border-b border-slate-50 px-4 py-2.5 text-[12px] last:border-0 hover:bg-slate-50 dark:border-slate-900 dark:hover:bg-slate-950/50">
                  ...
                  <div>
                    {editingKey === row.rawKey ? (
                      <div className="flex items-center gap-1">
                        <input
                          className="w-full rounded border border-cyan-400 bg-white px-1.5 py-0.5 text-[12px] focus:outline-none focus:ring-1 focus:ring-cyan-400 dark:bg-slate-900"
                          value={editValue}
                          onChange={(e) => setEditValue(e.target.value)}
                          onKeyDown={(e) => e.key === "Enter" && saveEdit(row.rawKey)}
                          autoFocus
                        />
                        <button onClick={() => saveEdit(row.rawKey)} className="text-[12px] font-semibold text-emerald-600">✓</button>
                        <button onClick={() => setEditingKey(null)} className="text-[12px] text-slate-400">✕</button>
                      </div>
                    ) : (
