  ...
  async function capture() {
    ...
    try {
      ...
      if (!res.ok || json.error) {
        setError(json.error ?? `Capture failed (${res.status})`);
        setCapturing(false);
        return;
      }
      ...
      const data: CaptureResult = {
        ...
        url: url.trim(),
        images: json.images ?? [],
        specs: json.specs ?? json.attributes ?? {},
        manuals: json.manuals ?? json.documents ?? [],
        capturedAt: json.created_at ?? new Date().toISOString(),
      };
      setResult(data);
    } catch (err: any) {
      ...
      <div className="relative mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-10 lg:py-8">
        ...
        <section className="space-y-2">
          ...
          <h1 className="text-xl font-semibold text-slate-900 sm:text-2xl dark:text-slate-50">
            ...
            <span className="bg-gradient-to-r from-cyan-400 via-sky-400 to-teal-300 bg-clip-text text-transparent dark:from-cyan-300 dark:via-sky-200 dark:to-teal-200">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-[0_16px_40px_rgba(148,163,184,0.25)] sm:p-5 dark:border-slate-800 dark:bg-slate-900/85">
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-200 bg-white py-2.5 pl-9 pr-3 text-sm placeholder:text-slate-400 focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/30 dark:border-slate-700 dark:bg-slate-950/80 dark:text-slate-100 dark:placeholder:text-slate-500"
              />
            </div>
            <button
              onClick={capture}
              disabled={capturing || !url.trim()}
              className="inline-flex items-center gap-1.5 rounded-lg bg-cyan-500 px-4 py-2.5 text-sm font-semibold text-white shadow-md shadow-cyan-500/40 transition hover:-translate-y-px hover:bg-cyan-400 disabled:opacity-60"
            >
              {capturing ? (
                <>
          ...
          <p className="mt-2 text-[12px] text-slate-400 dark:text-slate-500">
            Try: <button onClick={() => setUrl("https://medline.com/products/skil-care-mattress-overlay")} className="text-cyan-600 underline underline-offset-2">medline.com sample</button>
          </p>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
          ...
          <div>
            {!result && !capturing && (
              <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-slate-200 py-12 text-center dark:border-slate-800">
                ...
                <p className="text-[12px] text-slate-500">Paste a product URL above and click Capture.</p>
              </div>
            )}
            ...
            {result && !capturing && (
              <div className="rounded-2xl border border-slate-200 bg-white/95 shadow-[0_16px_40px_rgba(148,163,184,0.25)] dark:border-slate-800 dark:bg-slate-900/85 overflow-hidden">
                {/* Product header */}
                <div className="border-b border-slate-100 bg-cyan-50/60 px-5 py-4 dark:border-slate-800 dark:bg-cyan-500/5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-[12px] font-semibold uppercase tracking-wide text-slate-400">{result.brand} · {result.sku}</p>
                      <h3 className="mt-0.5 text-sm font-semibold text-slate-900 dark:text-slate-50">{result.name}</h3>
                      <p className="text-[12px] text-slate-500 dark:text-slate-400">{result.price}</p>
                    </div>
                    <span className="inline-flex items-center gap-1 rounded-full border border-emerald-300/60 bg-emerald-50 px-2 py-0.5 text-[12px] font-medium text-emerald-700 dark:border-emerald-500/40 dark:bg-emerald-500/10 dark:text-emerald-300">
                ...
                <div className="p-5 space-y-4">
                  {/* Specs */}
                  <div>
                    <h4 className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Specifications</h4>
                    <div className="divide-y divide-slate-50 rounded-lg border border-slate-200 dark:divide-slate-800 dark:border-slate-700">
                      {Object.entries(result.specs).map(([k, v]) => (
                        <div key={k} className="flex items-center justify-between px-3 py-1.5 text-[12px]">
                  ...
                  {/* Images */}
                  <div>
                    <h4 className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Images ({result.images.length})</h4>
                    <div className="flex gap-2">
                  ...
                  {result.manuals.length > 0 && (
                    <div>
                      <h4 className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Documents</h4>
                  ...
                  <div className="flex flex-wrap gap-2 border-t border-slate-100 pt-3 dark:border-slate-800">
                    <button onClick={sendToAvidia} className="inline-flex flex-1 items-center justify-center rounded-full bg-cyan-500 px-3 py-2 text-[12px] font-semibold text-white shadow-sm hover:bg-cyan-400">
                      Send to AvidiaExtract
                    </button>
                    <button onClick={downloadJSON} className="inline-flex flex-1 items-center justify-center rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-[12px] font-medium text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
                      Download JSON
                    </button>
                    <button onClick={downloadCSV} className="inline-flex flex-1 items-center justify-center rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-[12px] font-medium text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
                      Download CSV
                    </button>
                  </div>
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white/95 p-4 sm:p-5 dark:border-slate-800 dark:bg-slate-900/85">
              ...
              {history.length === 0 ? (
                <p className="text-[12px] text-slate-400 dark:text-slate-500">Captures sent to Avidia will appear here.</p>
              ) : (
                <div className="space-y-2">
                  {history.map((item) => (
                    <div key={item.id} className="flex items-start gap-2.5 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 text-[12px] dark:border-slate-800 dark:bg-slate-950/70">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[12px] text-slate-400">{item.brand} · {item.sku}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-3 space-y-1.5 text-[12px]">
                {["Install Chrome extension", "Log in with AvidiaTech account", "Browse any product page", "Click Capture → Send to Avidia"].map((step, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <span className="inline-flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-cyan-100 text-[12px] font-semibold text-cyan-700 dark:bg-cyan-500/20 dark:text-cyan-300">{i + 1}</span>
                    <span className="text-slate-600 dark:text-slate-300">{step}</span>
                  </div>
                ))}
              </div>
              <button className="mt-3 w-full rounded-lg border border-cyan-300/60 bg-cyan-50 py-2 text-[12px] font-medium text-cyan-700 transition hover:bg-cyan-100 dark:border-cyan-500/40 dark:bg-cyan-500/10 dark:text-cyan-300">
