...
function PlaceholderLogo({ name }: { name: string }) {
  return (
    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-[12px] font-bold uppercase tracking-wide text-slate-500 dark:bg-slate-800 dark:text-slate-400">
      {name.slice(0, 2)}
    </div>
  );
}
                ...
                const card = (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`inline-flex shrink-0 rounded-full px-2 py-0.5 text-[12px] font-semibold ${statusCls[item.status]}`}>
