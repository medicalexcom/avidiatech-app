...
export default function AvidiaSeoPage() {
    ...
    try {
      ...
      if (sourceMode === "ingestion") {
        ...
        if (reuseExistingWhenSameUrl && isSameAsInitial && ingestionIdParam) {
          ...
          // ensure we never pass null into a state typed as string (strictNullChecks)
  ...
  async function fetchBulkJob(jobId: string) {
    ...
    const json = await res.json().catch(() => null);
    if (!res.ok) throw new Error(json?.error?.message || json?.error || `Bulk fetch failed: ${res.status}`);
  ...
  async function fetchBulkItems(jobId: string, limit = 200) {
    ...
    const json = await res.json().catch(() => null);
    if (!res.ok) throw new Error(json?.error?.message || json?.error || `Bulk items fetch failed: ${res.status}`);
    const base = json?.data ?? json;
  ...
  async function submitBulkJob() {
    ...
    try {
      ...
      const res = await fetch("/api/v1/seo/bulk", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
      const json = await res.json().catch(() => null);
  ...
  const rawDescriptionHtml = (jobData as any)?.descriptionHtml ?? (jobData as any)?.description_html ?? (jobData as any)?._debug?.description_html ?? null;
  ...
  const knownSeoKeys = [
    "h1","pageTitle","title","metaDescription","meta_description","seoShortDescription","seo_short_description","shortDescription","short_description","keywords","slug","name_best"
  ];
  ...
  // derived preview for the pasted rows
  const bulkRowsPreview = bulkRows.map((r) => ({
    key: r.idempotencyKey,
    url: r.url,
    price: r.price,
    domain: r.domain,
    warning: r.reason ?? undefined,
    valid: r.valid,
  }));

  return (
    <main className="relative min-h-[calc(100vh-64px)]">
      {/* Page background & hero (kept as earlier) */}
      <div className="mx-auto w-full max-w-7xl px-4 py-6 md:px-6 lg:px-8">
        {/* ... hero & command UI above omitted here for brevity in this block (kept in file) ... */}
        {/* For deploy, full UI is included above in this file (we preserved content). */}
        {/* RESULTS & PANELS */}
        <div className="mt-6 space-y-6">
          {/* (Single & Bulk panels) -- already rendered above */}
          {/* After bulk preview table, controls */}
          <div className="rounded-2xl border border-slate-200 bg-white/70 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/40">
            <div className="flex items-center justify-between">
              <div className="text-sm font-semibold">Bulk preview</div>
              <div className="text-xs text-slate-500">Preview first 200 rows</div>
            </div>

            <div className="mt-3">
              <div className="flex items-center gap-2">
                <button className="rounded bg-cyan-600 px-3 py-1 text-white" onClick={submitBulkJob} disabled={bulkSubmitting}>
                  {bulkSubmitting ? "Submitting…" : "Create bulk job"}
                </button>
                <button className="rounded border px-3 py-1" onClick={() => { setBulkText(""); setBulkRows([]); setBulkRemoved({}); }}>
                  Clear
                </button>
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {bulkRowsPreview.length ? (
                    bulkRowsPreview.slice(0, 200).map((r) => (
                      <tr key={r.key} className="border-t border-slate-200 dark:border-slate-800">
                        <td className="p-2">
                          <input type="checkbox" checked={!bulkRemoved[r.key]} onChange={() => setBulkRemoved((p) => ({ ...p, [r.key]: !p[r.key] }))} />
                        </td>
                        <td className="p-2">
                          <div className="max-w-[460px] truncate font-mono text-xs text-slate-800 dark:text-slate-200">{r.url}</div>
                        </td>
                        <td className="p-2 text-center">{r.price ?? "—"}</td>
                        <td className="p-2 text-center">{r.domain ?? "—"}</td>
                        <td className="p-2 text-xs">{r.warning ?? (r.valid ? "OK" : "Invalid")}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td className="p-4 text-sm text-slate-500" colSpan={5}>No rows parsed yet. Paste URLs or upload a CSV.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          {/* Description HTML / SEO / Telemetry area */}
              ...
              <div className="rounded-2xl border bg-white p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">Rendered SEO description</div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <button className="rounded border px-2 py-1 text-sm" onClick={handleCopyDescription} disabled={!descriptionHtml}>{copyState === "copied" ? "Copied" : "Copy"}</button>
                  </div>
                </div>
                  ...
                  <>
                    ...
                    <div className="mt-3 overflow-auto">
                      <table className="min-w-[640px] w-full text-sm">
                        <thead><tr className="border-b"><th className="p-2 text-left">Module</th><th className="p-2">Status</th><th className="p-2">Duration</th><th className="p-2">Output</th></tr></thead>
                        <tbody>
                          {(pipelineSnapshot.modules ?? []).map((m) => (
                            <tr key={`${m.module_index}-${m.module_name}`} className="border-b">
                              ...
                              <td className="p-2">{m.started_at && m.finished_at ? formatDuration(safeDateMs(m.finished_at)! - safeDateMs(m.started_at)!) : m.status === "running" && m.started_at ? formatDuration(Date.now() - (safeDateMs(m.started_at) || Date.now())) : "—"}</td>
