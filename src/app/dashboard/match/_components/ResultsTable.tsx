...
export default function ResultsTable({ rows = [], onConfirm, onReject, onIngest }: Props) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      {/* Header */}
      <div className="grid grid-cols-[minmax(0,1fr)_auto_auto_minmax(0,1.8fr)_auto_auto] items-center border-b border-slate-100 bg-slate-50/80 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500 gap-3 dark:border-slate-800 dark:bg-slate-950/60 dark:text-slate-400">
        <span>SKU</span>
        <span>Brand</span>
        ...
        {localRows.map((row) => (
          <div key={row.id} className="grid grid-cols-[minmax(0,1fr)_auto_auto_minmax(0,1.8fr)_auto_auto] items-center px-4 py-3 gap-3 text-xs">
            <div className="min-w-0">
              ...
              {row.product_name && <p className="text-[11px] text-slate-400 truncate">{row.product_name}</p>}
            </div>
            ...
            <div className="flex gap-1 shrink-0">
              {row.status === "pending" && (
                <>
                  <button
                    ...
                    className="rounded-lg bg-emerald-50 border border-emerald-200 px-2 py-1 text-[10px] font-medium text-emerald-700 hover:bg-emerald-100 disabled:opacity-60 dark:border-emerald-500/30 dark:bg-emerald-950/30 dark:text-emerald-300"
                  >
                    Confirm
                  </button>
                  <button
                    ...
                    disabled={actingId === row.id}
                    className="rounded-lg border border-slate-200 px-2 py-1 text-[10px] font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-60 dark:border-slate-700 dark:text-slate-300"
                  >
                    Reject
                  </button>
                </>
              ...
              {row.status === "confirmed" && (
                <button
                  ...
                  disabled={actingId === row.id}
                  className="rounded-lg bg-sky-50 border border-sky-200 px-2 py-1 text-[10px] font-medium text-sky-700 hover:bg-sky-100 disabled:opacity-60 dark:border-sky-500/30 dark:bg-sky-950/30 dark:text-sky-300"
                >
                  Ingest
                </button>
              )}
