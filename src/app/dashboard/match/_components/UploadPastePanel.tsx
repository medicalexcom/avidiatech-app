    ...
    <div className="space-y-4">
      ...
      >
        <div className="px-4 pt-3 pb-1 flex items-center justify-between gap-2">
          <p className="text-[12px] font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
            Paste SKUs or drop a CSV
          </p>
          <button
            type="button"
            onClick={() => fileInputRef.current?.click()}
            className="text-[12px] font-medium text-sky-600 hover:text-sky-500 dark:text-sky-400"
          >
            Browse file
          </button>
          <input
      ...
      {warnings.length > 0 && (
        <div className="rounded-xl border border-amber-200 bg-amber-50/80 px-3 py-2.5 dark:border-amber-500/30 dark:bg-amber-950/30">
          <p className="text-[12px] font-semibold text-amber-700 dark:text-amber-300">Parse warnings</p>
          <ul className="mt-1 space-y-0.5 text-[12px] text-amber-700 dark:text-amber-300">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white/80 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/70 space-y-3">
          <div className="flex items-center justify-between gap-2">
            <div>
              ...
              <p className="text-[12px] text-slate-500 dark:text-slate-400 font-mono">{job.jobId}</p>
            </div>
            <span className={`rounded-full border px-2 py-0.5 text-[12px] font-semibold uppercase tracking-wide ${
          ...
          {/* Progress bar */}
          <div>
            <div className="flex justify-between text-[12px] text-slate-500 dark:text-slate-400 mb-1">
          ...
          {job.results.length > 0 && (
            <div className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800">
              <div className="grid grid-cols-[minmax(0,1fr)_auto_auto] text-[12px] font-semibold uppercase tracking-[0.14em] text-slate-500 bg-slate-50 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800 px-3 py-1.5">
                <span>Row ID</span>
                <span>Status</span>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800/80 max-h-48 overflow-y-auto">
                {job.results.slice(0, 20).map((r) => (
                  <div key={r.row_id} className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center px-3 py-1.5 text-xs">
                    ...
                    <span className={`text-[12px] font-medium ${r.status === "matched" ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400"}`}>
                      {r.status}
                    </span>
          ...
          {job.error && (
            <p className="text-[12px] text-rose-600 dark:text-rose-400">{job.error}</p>
          )}
        </div>
      )}
    </div>
