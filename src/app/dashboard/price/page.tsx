...
/**
 * Combined layout:
 * - CalculatorCard rendered once (full-width card above the workspace)
 * - PriceWorkspace below (Recent ingestions). The workspace may expose a selected ingestion event
 *   via props/callback that the CalculatorCard can use to preview computed prices.
 *
 * If you want CalculatorCard to preview the selected ingestion, update PriceWorkspace to accept
 * an `onSelect` callback that provides the selected ingestion object; pass that into CalculatorCard as a prop.
 */
      ...
      <div className="relative px-4 py-6 sm:px-6 lg:px-10 lg:py-8 max-w-7xl mx-auto space-y-6">
        ...
        <section className="mb-2 flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-4 max-w-2xl flex-1 min-w-[260px]">
            {/* title and description preserved (omitted here for brevity) */}
        ...
        {/* Combined Calculator card above the workspace (single instance) */}
        ...
        {/* Price workspace (Recent ingestions) */}
        <PriceWorkspace />
      </div>
    </main>
  );
