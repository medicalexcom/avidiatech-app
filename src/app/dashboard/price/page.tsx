 ...
 *
 * This edit mounts the CalculatorCard in the right-hand column so the
 * "Calculator" advanced UI (JS formula / Legacy tier editor / Basic toggles)
 * is available from the pricing page.
 */
...
export default function PricePage() {
  // tenantId can be passed here if you have it client-side;
  // passing null loads the global (storewide) formula.
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-50 relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0">
        ...
        <div className="absolute inset-0 opacity-[0.04] dark:opacity-[0.06] mix-blend-soft-light">
          <div className="h-full w-full bg-[linear-gradient(to_right,#e5e7eb_1px,transparent_1px),linear-gradient(to_bottom,#e5e7eb_1px,transparent_1px)] bg-[size:46px_46px] dark:bg-[linear-gradient(to_right,#0f1724_1px,transparent_1px),linear-gradient(to_bottom,#0f1724_1px,transparent_1px)] dark:bg-[size:46px_46px]" />
        </div>
      </div>
        ...
        <section className="mb-2 flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="space-y-4 max-w-2xl flex-1 min-w-[260px]">
            ...
            <div className="flex flex-wrap gap-3 text-[11px]">
              <div className="inline-flex items-center gap-2 rounded-full bg-white/90 border border-emerald-300/70 px-3 py-1.5 text-slate-700 shadow-sm dark:bg-slate-950/90 dark:border-emerald-500/35">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />
                <span>Server-authoritative compute + audit trail.</span>
              </div>
              <div className="inline-flex items-center gap-2 rounded-full bg-white/90 border border-sky-300/70 px-3 py-1.5 text-slate-700 shadow-sm dark:bg-slate-950/90 dark:border-sky-500/35">
          ...
          {/* Right column: keep status card and add CalculatorCard below it */}
          <div className="w-full max-w-xs lg:max-w-sm mt-1 lg:mt-0 space-y-3">
            <div className="rounded-2xl border border-slate-200 bg-white/95 px-4 py-3 sm:px-5 sm:py-4 space-y-3 shadow-md shadow-slate-200/70 dark:border-slate-800 dark:bg-slate-950/90 dark:shadow-slate-900/40">
            ...
            {/* Calculator card: advanced UI (Basic / Advanced / Legacy) */}
