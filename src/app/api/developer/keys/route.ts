...
export async function GET(request: Request) {
  const rl = await checkRateLimit("developer", getClientIP(request));
...
export async function POST(request: Request) {
  const rl2 = await checkRateLimit("developer", getClientIP(request));
  if (!rl2.success) return rateLimitResponse(rl2);
