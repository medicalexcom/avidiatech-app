...
export async function PATCH(req: Request, context: any) {
  try {
    ...
    const { data, error } = await getServerSupabase().from("monitor_rules").update(allowed).eq("id", id).select("*").maybeSingle();
...
export async function DELETE(req: Request, context: any) {
  try {
    ...
    const { error } = await getServerSupabase().from("monitor_rules").delete().eq("id", id);
    if (error) throw error;
