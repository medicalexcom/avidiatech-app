...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("monitor_watches").update(allowed).eq("id", params.id).select("*").maybeSingle();
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { error } = await supabaseAdmin.from("monitor_watches").delete().eq("id", params.id);
    if (error) throw error;
