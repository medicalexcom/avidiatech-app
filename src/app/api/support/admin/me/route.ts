...
export async function GET(req: Request) {
  try {
    ...
    const { data: profile } = await supabase.from("profiles").select("full_name, email").eq("user_id", userId).limit(1).maybeSingle();
    return NextResponse.json({ isAgent: !!agentCheck, userId, name: profile?.full_name ?? null, email: profile?.email ?? null });
  } catch (err: any) {
