...
export async function POST(req: Request, context: any) {
  try {
    ...
    try {
      ...
      const { error: upsertErr } = await getServerSupabase()
    ...
    try {
      const { data: job } = await getServerSupabase()
        .from("match_url_jobs")
      ...
      if (job) {
        ...
        await getServerSupabase().from("match_url_jobs").update({ resolved_count: newResolved, updated_at: now2 }).eq("id", jobId);
      }
    } catch (err) {
