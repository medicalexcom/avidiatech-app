...
async function ensureAutoMapping(orgId: string) {
  ...
  try {
    const { error } = await getServerSupabase().from(AUTO_MAPPING_TABLE).insert([payload]);
...
export async function POST(req: Request) {
  try {
    ...
    const { data: job, error: jobErr } = await getServerSupabase().from("match_url_jobs").insert([jobPayload]).select("*").maybeSingle();
    if (jobErr) {
    ...
    if (inserts.length) {
      const { error: insErr } = await getServerSupabase().from("match_url_job_rows").insert(inserts);
