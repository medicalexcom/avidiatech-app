...
export async function POST(req: Request, context: any) {
  try {
    ...
    const { data: runRow, error: runErr } = await getServerSupabase().from("pipeline_runs").select("*").eq("id", pipelineRunId).single();
    ...
    await getServerSupabase().from("pipeline_runs").update({ status: "retry_requested", updated_at: new Date().toISOString() }).eq("id", pipelineRunId);
