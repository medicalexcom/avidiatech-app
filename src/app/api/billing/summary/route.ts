 ...
 *   trialEnd: string | null         // ISO date if trialing
 ...
 *     translations: { used: number; limit: number | null }  // alias for seo for BillingPanel compat
 *   }
 * }
 */
export async function GET(req: Request) {
  try {
    ...
    // ── Resolve tenantId ────────────────────────────────────────────────────
    // For self-serve, tenantId = userId. Check team_members first.
    ...
    const status      = sub?.status ?? (isOwner ? "owner-bypass" : "none");
    ...
    return NextResponse.json({
      ...
      usage: {
        ...
        // alias for BillingPanel which currently reads `translations`
