...
export default function SignInPage() {
  ...
  useEffect(() => {
    // if Clerk hasn't initialized in 4s, show fallback
    const t = setTimeout(() => {
      setClerkFailed(true);
      // attempt to fetch the /v1/client endpoint to get any server error message for debugging
      (async () => {
        try {
          const frontendApi = process.env.NEXT_PUBLIC_CLERK_FRONTEND_API || "";
          // build the same URL the browser requests (Clerk client endpoint)
