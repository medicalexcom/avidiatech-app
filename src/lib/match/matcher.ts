...
function domainFromMaybeUrl(u?: string | null) {
  if (!u) return null;
 ...
 * 0) Use job-provided manufacturer_domain/manufacturer_url if present (authoritative).
...
async function resolveManufacturerDomain(row: any): Promise<string | null> {
  // 0) Job-provided domain wins
  const provided =
    normalizeDomain(row?.manufacturer_domain) ??
    domainFromMaybeUrl(row?.manufacturer_url) ??
    domainFromMaybeUrl(row?.manufacturerUrl);
  ...
  try {
    ...
    if (Array.isArray(data) && data.length) {
      const candidates = data
        .map((d: any) => (typeof d.source_domain === "string" ? d.source_domain.replace(/^www\./, "").toLowerCase() : null))
        .filter(Boolean);
...
export async function processRow(row: any) {
  ...
  // Resolve manufacturer domain first
  ...
  if (!best || best.score < VALIDATION_THRESHOLD) {
    ...
    await getServerSupabase()
      ...
      .update({
        ...
        status: "unresolved",
        confidence: Math.max(...candidateRecords.map((c) => c.score), 0),
  ...
  try {
    ...
    await getServerSupabase().from("product_source_index").upsert(payload, { onConflict: "tenant_id,supplier_key,sku_norm" });
  } catch (err) {
    console.warn("product_source_index upsert failed:", err);
  }

  return { ok: true, accepted: acceptedCandidate, manufacturer_domain: manufacturerDomain };
}
