import { SUPPLIER_CONFIG } from "./connectors/config";
...
export function allowlistedDomainsForSupplier(supplierKey: string): string[] {
  const key = (supplierKey || "").toString().toLowerCase();
  const cfg = (SUPPLIER_CONFIG as any)[key];
  const allow = (cfg?.allowDomains ?? []) as string[];
  return allow.map((d) => d.toLowerCase());
}
