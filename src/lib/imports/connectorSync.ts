...
export default async function processConnectorSync(jobId: string, integration: any) {
  // decrypt secrets
  ...
  if (provider === "bigcommerce") {
    ...
    if (inserts.length) {
      ...
      for (let i = 0; i < inserts.length; i += chunk) {
        ...
        const { error } = await getServerSupabase().from("import_rows").insert(part);
        if (error) {
          throw error;
        }
      }
  ...
  await getServerSupabase().from("import_jobs").update({
    status: "failed",
