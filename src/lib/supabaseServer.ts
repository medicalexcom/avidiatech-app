// Modified supabaseServer helper to include org_id in insert payload
//
// This version of saveIngestion ensures that every ingestion row records both
// tenant_id and org_id.  It derives org_id from the resolved tenant key, so
// callers do not need to change existing code paths.  Without this field, the
// NOT NULL constraint on product_ingestions.org_id will cause inserts to fail.
...
if (!url || !serviceKey) {
  console.warn(
    "Supabase service role or URL not configured. Supabase helpers will no-op when used."
  );
}
...
function resolveTenantKeyStrict(
 ...
 * - org_id is automatically derived from tenantId and included in the insert payload.
...
export async function saveIngestion({
  ...
  const payload: Record<string, any> = {
    tenant_id: tenantKey,
    org_id: tenantKey, // ensure org_id always matches tenant
    user_id: userId ?? null,
    type,
    status,
