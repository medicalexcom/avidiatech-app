 ...
 * Environment:
 * - SUPABASE_URL
 * - SUPABASE_SERVICE_ROLE_KEY (used by getServerSupabase() to write notifications)
...
export async function processEvent(eventRow: any) {
  ...
  const { data: rules } = await getServerSupabase()
    .from("monitor_rules")
    .select("*")
    ...
    try {
      ...
      if (action.type === "webhook" && action.url) {
        ...
        if (action.webhook_id) {
          const { data: wh } = await getServerSupabase().from("monitor_webhooks").select("*").eq("id", action.webhook_id).limit(1).maybeSingle();
          secret = wh?.secret;
        }
        ...
        await getServerSupabase().from("monitor_notifications").insert([{
  ...
  await getServerSupabase().from("monitor_events").update({ processed: true }).eq("id", eventRow.id);
...
export async function processPendingEvents(limit = 50) {
  const { data: events, error } = await getServerSupabase()
    .from("monitor_events")
    .select("*")
    .eq("processed", false)
