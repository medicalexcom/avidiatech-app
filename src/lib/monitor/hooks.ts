...
export async function createWatchForIngestion(payload: {
  ...
  if (existing) {
    if (payload.product_id && !existing.product_id) {
      await getServerSupabase().from("monitor_watches").update({ product_id: payload.product_id }).eq("id", existing.id);
    }
    if (payload.tenant_id && !existing.tenant_id) {
      await getServerSupabase().from("monitor_watches").update({ tenant_id: payload.tenant_id }).eq("id", existing.id);
    }
  ...
  const { data, error } = await getServerSupabase().from("monitor_watches").insert([insert]).select("*").maybeSingle();
